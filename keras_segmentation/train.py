...
def train( model  ,
    ...
    if not validate:
        for ep in range(epochs):
            ...
            ker_history = model.fit_generator(train_gen, steps_per_epoch, epochs=1, use_multiprocessing=True)

            hist['acc'].append(ker_history.history['acc'][0])
            hist['val_acc'].append(ker_history.history['val_acc'][0])
        ...
        for ep in range(epochs):
            print("Starting Epoch ", ep)
            ker_history = model.fit_generator(train_gen, steps_per_epoch, validation_data=val_gen,
